"""
Módulo Alternância Circular (Round-Robin) com Gerenciamento de E/S
====================================================================
Implementa o escalonamento Round-Robin com suporte a operações de E/S,
...
class AlternanciaCirc:
    """
    Implementa o algoritmo de escalonamento Round-Robin (Alternância Circular)
    com gerenciamento de E/S.
    """
    ...
    def iniciar(self):
        """Inicia a execução do escalonador Round-Robin."""
        print("Round-Robin (Alternância Circular) começou")
        self._executar()
        print("Round-Robin terminou")
    ...
    def _executar(self):
        """Executa o escalonador Round-Robin com gerenciamento de E/S."""
        processos_aguardando = self.todos_processos.copy()
        fila_rr = []
        
        while (processos_aguardando or self.processos_prontos or 
               self.processos_executando or self.processos_bloqueados or fila_rr):
            ...
            while self.processos_prontos:
                ...
                if proc not in fila_rr:
                    fila_rr.append(proc)
            
            if not self.processos_executando and fila_rr:
                proximo = fila_rr.pop(0)
            ...
            if self.processos_executando:
                ...
                if disp_es and momento_req >= 0 and momento_req < tempo_fatia:
                    ...
                    if proc_atual.tempo_execucao_restante <= 0:
                        ...
                        self.processos_executando.remove(proc_atual)
                        fila_rr.append(proc_atual)
                        print(f"\n⏹️  Clock {self.gerenciador.clock}: Processo {proc_atual.pid} saiu da CPU (volta para fim da fila)")
