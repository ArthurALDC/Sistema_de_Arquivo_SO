...
class FIFO:
    ...
    def verificar_requisicao_es(self, processo):
        ...
        if random.random() < processo.chance_req_es:
            ...
            # No FIFO, como não há quantum, o momento da requisição é sorteado 
            # dentro do tempo restante do processo para esta fatia de execução.
            momento_req = random.randint(0, processo.tempo_execucao_restante - 1)
    ...
    def _executar(self):
        ...
        while (processos_aguardando or self.processos_prontos or 
            ...
            # No FIFO, não há quantum. O processo assume a CPU até terminar ou pedir E/S.
            ...
            # 5. Executar o processo atual
            if self.processos_executando:
                proc_atual = self.processos_executando[0]
                
                # Avança a memória a cada tick de CPU
                self.dict_process_mem[proc_atual.pid].main_call()
                
                # Verifica se haverá requisição de E/S durante a execução restante
                disp_es, momento_req = self.verificar_requisicao_es(proc_atual)
                
                if disp_es is not None and momento_req >= 0 and momento_req < proc_atual.tempo_execucao_restante:
                    ...
                    # O processo executa até o fim (não há preempção por tempo no FIFO)
                    tempo_a_executar = proc_atual.tempo_execucao_restante
                    print(f"\nClock {self.gerenciador.clock}: Processo {proc_atual.pid} executando por {tempo_a_executar} unidades")
                    
                    proc_atual.tempo_execucao_restante = 0
                    proc_atual.tempo_executando += tempo_a_executar
                    self.gerenciador.clock += tempo_a_executar
